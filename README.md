...
The local Web UI and API use HTTP port 80; live status uses WebSocket port 81. Dashboard, Scope and System status are functional. Events and Sessions are placeholders. Scope data is reduced on the ESP to 160 min/max columns; the full 50,000-sample ring remains in RAM. No external web assets are loaded.
...
The Primary profile connected to a local Wi-Fi network after provisioning and again after firmware flash. HTTP Start/Stop, manual ARM/trigger/capture, and 35,000/15,000 pre/post placement passed. A 10-minute HTTP plus WebSocket load check with Dashboard and 160-column Scope polling kept ADC near 83,333 samples/s with zero overruns, zero ADC read errors and no reboot. One HTTP request timed out during polling; the final Stop command executed although its response timed out. The board reported a 124,908-byte minimum heap during the session. Desktop and mobile Chrome render checks passed after the mobile grid fix.
